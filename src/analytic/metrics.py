...
Two layers, kept separate so the aggregation math can be unit-tested independently of the validation thresholds:
    ...
    - `check_reqs(nodes, ...)` evaluates the R1 / R2 targets against the thresholds declared in a reference file (default: `data/reference/baseline.json`, which carries the Camara 2023 values):

        R1  failure rate   <= 0.03 %   (Availability)
        R2  response time  <= 26 ms    (Performance)

Thresholds are NOT hardcoded; they live in `data/reference/<name>.json` so the case-study ground truth can be edited without touching Python.
"""
...
def check_reqs(nodes: pd.DataFrame,
    ...
    """*check_reqs()* evaluates the R1 / R2 targets against the thresholds declared in a reference file, using either caller-supplied values or values derived from the per-node frame.
    ...
    Returns:
        Dict[str, Dict[str, Any]]: verdicts keyed by `R1`, `R2`. Each verdict carries `metric`, `value`, `threshold`, `operator`, `units`, `pass`, and `notes`.
    """
