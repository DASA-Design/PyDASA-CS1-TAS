...
Runs one symbolic-differentiation sweep at a single evaluation point (mean,
setpoint, min, or max) and reshapes the raw output into a numeric-only
nested dict.

    - `analyse_symbolic(engine, schema, val_type="mean")` returns `{SEN_{coeff}: {var: float}}`.

*IMPORTANT:* pydasa keys sensitivity entries as `SEN_{<coeff_symbol>}`, so
raw Pi entries become `SEN_{\\Pi_{0}}` and derived ones e.g. `SEN_{\\theta_{TAS_{1}}}`. Callers that want the raw coefficient symbol back need to strip the prefix.
"""
...
def analyse_symbolic(engine: AnalysisEngine,
    ...
    """*analyse_symbolic()* runs symbolic sensitivity at a single value type.
