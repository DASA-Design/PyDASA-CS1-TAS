...
Key design choices (see `notes/experiment.md` for rationale):
    ...
    - **Deterministic interarrival** (`await asyncio.sleep(1/rate)`). Poisson emerges naturally from downstream routing and service-time variability; not enforced here.
    - **Kind weights from the profile routing matrix**. The CLIENT samples the request kind per TAS_{1}'s routing-row probabilities; the architecture routes deterministically on `req.kind`.
    - **Sample-count probes**. At each rate in the schedule, send until every kind has >= `min_samples_per_kind` completed requests (or a safety timeout hits), then step to the next rate.
    - **Cascade stop on infrastructure failures only**. Business failures (HTTP 200 + `success=False`) are counted but do NOT stop the ramp -- they are the adaptation-target signal we want to measure. Only 503 / timeout / 5xx trigger stop, per the configured cascade rule (rolling-window or fail-fast).
    - **No cost field**. Cost is a domain concept (R3); the client records only what's architecturally observable.

    - `ClientConfig(entry_service, kind_weights, ramp)` is the whole runtime config.
...
from src.experiment.payload import size_for_kind as _size_for_kind
...
class InvocationRecord:
    ...
    Fields are architecturally observable only; domain concepts (cost) live elsewhere. Three failure-mode flags are derived from `(status_code, success)` via helper properties so downstream analysis
    never has to guess.
    """
    ...
    send_ts: float = 0.0
    recv_ts: float = 0.0
    status_code: int = 0       # HTTP status; -1 means transport exception (timeout etc.)
    success: bool = False       # body.success -- business-level outcome
    size_bytes: int = 0
...
class CascadeConfig:
    ...
    Attributes:
        mode: `"rolling"` (threshold over a rolling window) or `"fail_fast"` (stop on any single infra failure).
        threshold: rolling-mode only. infra-fail rate above this -> stop.
        window: rolling-mode only. size of the trailing window (number of requests).
    """
...
class ClientConfig:
    ...
    `kind_weights` maps kind labels (arbitrary strings chosen at the launcher
    level when it inspects TAS_{1}'s routing row) to probabilities summing to 1.
    """

    entry_service: str = "TAS_{1}"
    seed: int = 42
    # fallback size used when request_sizes_by_kind has no entry for a kind
    request_size_bytes: int = 256
    # FR-2.3: per-kind payload-size map, read directly from the method
    # config's `request_size_bytes` block. Keys either match kind labels
    # exactly (e.g. `TAS_{2}`) or use the `<kind>_request` alias form
    # (e.g. `analyse_request`). `size_for_kind()` resolves both.
...
def validate_ramp(ramp: Dict[str, Any]) -> None:
    """*validate_ramp()* validate the `ramp` sub-dict of `experiment.json`.

    Raises `ValueError` when any knob is out of the supported range.
    """
...
def build_ramp_cfg(ramp: Dict[str, Any]) -> RampConfig:
    """*build_ramp_cfg()* convert the `ramp` dict from `experiment.json` into a `RampConfig`; validates first."""
    validate_ramp(ramp)
    _cas = ramp.get("cascade", {})
    return RampConfig(
...
class _CascadeDetector:
    ...
    Call `observe(rec)` on each completed invocation; read `tripped` after
    each observation to decide whether to stop the ramp.
    """
...
class ClientSimulator:
    ...
    Attributes:
        _client: shared httpx AsyncClient (routed to the in-process mesh or real TCP).
        _registry: name -> URL resolver.
        _cfg: ClientConfig.
    """
    ...
    def _pick_kind(self) -> str:
        """*_pick_kind()* weighted draw from kind_weights (client side, per user spec)."""
    ...
    async def _send_one(self, kind: str) -> InvocationRecord:
        """*_send_one()* send one kind-tagged request to the entry service; return an InvocationRecord.
        ...
        """
        # resolve per-kind payload size from the method config's
        # request_size_bytes block; falls back to the scalar default
        _size = _size_for_kind(self._cfg.request_sizes_by_kind,
                               kind,
                               default=int(self._cfg.request_size_bytes))
        # generate a real byte payload using the client's seeded RNG so
        ...
        _url = self._registry.invoke_url(self._cfg.entry_service)
    ...
    async def _probe_at_rate(self,
        ...
        """*_probe_at_rate()* send at `rate` req/s until every kind has >= `min_samples_per_kind` or the probe-window timeout / cascade fires.

        Args:
            rate (float): deterministic target rate (req/s).
            detector (_CascadeDetector): shared cascade state.

        Returns:
            Dict[str, Any]: probe summary with `{rate, duration_s, samples_per_kind, stats_per_kind, infra_fail_rate, business_fail_rate, stopped_reason}`.
        """
        ...
        async def _collect(task: asyncio.Task) -> None:
            """*_collect()* await one in-flight task; record + update counters + cascade."""
            _rec = await task
        ...
        # drain any remaining in-flight tasks (bounded by the individual timeouts)
    ...
    async def run_ramp(self) -> Dict[str, Any]:
        """*run_ramp()* execute the full ramp schedule; stop on cascade.
