...
async def _demo() -> None:
    ...
    # -------- 1. SvcSpec ---------------------------------------------
    _banner("1. SvcSpec - frozen per-service knobs (no queueing logic)")
    _spec = SvcSpec(name="MAS_{1}",
                    role="atomic",
                    port=8006,
                    mu=500.0,
                    epsilon=0.1,
    ...
    print(f"  MEM_HEADROOM_FACTOR  = {SvcSpec.MEM_HEADROOM_FACTOR}")
    ...
    # -------- 3. SvcCtx ------------------------------------------
    _banner("3. SvcCtx - mutable runtime state (spec + rng + log)")
    ...
    _ctx2 = SvcCtx(spec=SvcSpec(name="TAS_{2}",
    ...
    # -------- 5. mount_atomic_svc ------------------------------------
    _banner("5. mount_atomic_svc - terminal MAS handler (empty routing)")
    _app = make_base_app("demo::MAS")
    _ctx3 = mount_atomic_svc(
        _app,
        SvcSpec(name="MAS_{1}",
                role="atomic",
                port=8007,
    ...
    async with httpx.AsyncClient(transport=_transport,
        ...
        _r = await _c.post("/invoke",
                           json=SvcReq(kind="analyse",
                                       size_bytes=64).model_dump())
    print(f"  POST /invoke  status = {_r.status_code}")
    print(f"  body                 = {_r.json()}")
    print(f"  ctx.log rows         = {len(_ctx3.log)}")

    # -------- 6. mount_composite_svc ---------------------------------
    _banner("6. mount_composite_svc - TAS-like: entry + internal hop + external forward")
    ...
    async with httpx.AsyncClient(transport=_transport,
                                 base_url="http://t") as _c:
        _req = SvcReq(kind="TAS_{2}", size_bytes=128)
