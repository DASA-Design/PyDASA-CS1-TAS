...
class TestJacksonSolver:
    """**TestJacksonSolver** verifies `solve_jackson_lams()` returns per-node arrival rates expected from flow conservation on small, hand-checkable topologies."""
    ...
    def test_two_node_feedforward(self):
        ...
        _P = np.array([[0.0, 1.0],
                       [0.0, 0.0]])
        _lambdas = solve_jackson_lams(_P, [10.0, 0.0])
    ...
    def test_two_node_split(self):
        ...
        _P = np.array([[0.0, 0.7],
                       [0.0, 0.0]])
        _lambdas = solve_jackson_lams(_P, [100.0, 0.0])
    ...
    def test_no_external_no_flow(self):
        ...
        _P = np.array([[0.0, 0.5],
                       [0.5, 0.0]])
        _lambdas = solve_jackson_lams(_P, [0.0, 0.0])
    ...
    def test_shape_preserved(self):
        ...
        _P = np.zeros((13, 13))
        _lambdas = solve_jackson_lams(_P, np.zeros(13))
