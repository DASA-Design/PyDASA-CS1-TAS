...
`SvcCtx` (own CSV log, own seeded RNG). TAS-to-TAS hops are
...
def _tas_spec(name: str, *, mu: float = 1000.0, epsilon: float = 0.0,
    ...
    """*_tas_spec()* build a `SvcSpec` for a TAS member with stock defaults."""
    return SvcSpec(name=name, role="composite", port=8001,
...
class TestTasComponentIsolation:
    """**TestTasComponentIsolation** each TAS member has its own `SvcCtx` + log buffer."""

    def test_app_exposes_per_component_states(self, _app_internal_only):
        """*test_app_exposes_per_component_states()* `app.state.tas_components` has one distinct `SvcCtx` per declared member."""
