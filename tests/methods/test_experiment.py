...
class TestResultEnvelope:
    ...
    def test_wrt_true_writes_json(self, tmp_path, monkeypatch):
        ...
        # output path now includes the deployment axis (default `localhost`)
        _path = (tmp_path / "experiment" / "localhost" / "baseline" / "dflt.json")
        assert _path.exists()
...
class TestDeploymentAxis:
    ...
    def test_default_deployment_is_localhost(self, tmp_path, monkeypatch):
        """*test_default_deployment_is_localhost()* with no `dpl` arg and no method_cfg override, the run defaults to `localhost` mode."""
        from src.methods import experiment as _mod
        monkeypatch.setattr(_mod, "_RESULTS_DIR", tmp_path / "experiment")
        _res = run_experiment(adp="baseline", wrt=False,
                              method_cfg=_QUICK_CFG,
                              skip_calibration=True, verbose=False)
        assert _res["deployment"] == "localhost"
        assert _res["launcher_role"] == "all"

    def test_explicit_dpl_localhost_is_accepted(self, tmp_path, monkeypatch):
        """*test_explicit_dpl_localhost_is_accepted()* passing `dpl="localhost"` explicitly produces the same envelope shape as the default."""
        from src.methods import experiment as _mod
        monkeypatch.setattr(_mod, "_RESULTS_DIR", tmp_path / "experiment")
        _res = run_experiment(adp="baseline", wrt=False,
                              method_cfg=_QUICK_CFG,
    ...
    def test_non_localhost_dpl_raises_not_implemented(self, tmp_path,
                                                      monkeypatch):
        """*test_non_localhost_dpl_raises_not_implemented()* `dpl='multiprocess'` propagates `NotImplementedError` from the launcher gate (until distribute G5 lands the real-uvicorn path)."""
        from src.methods import experiment as _mod
        monkeypatch.setattr(_mod, "_RESULTS_DIR", tmp_path / "experiment")
    ...
    def test_output_path_includes_deployment_segment(self, tmp_path,
                                                     monkeypatch):
        """*test_output_path_includes_deployment_segment()* `wrt=True` writes under `<results>/localhost/<scenario>/<profile>.json` (deployment axis appears between results root and scenario)."""
        from src.methods import experiment as _mod
        monkeypatch.setattr(_mod, "_ROOT", tmp_path)
        monkeypatch.setattr(_mod, "_RESULTS_DIR", tmp_path / "experiment")
        ...
        assert "localhost" in _res["paths"]["profile"]
